...
public final class TqstubConverter {

    public static final String TYPE_NAME = "TQSTUB";

    private TqstubConverter() {
    }

    public static Struct getStruct(Tqstub o, Connection connection) throws SQLException {
    if (o == null) {
        return null;
    }
    ...
    public static Array getListArray(List<Tqstub> o, Connection c, String typeName) throws SQLException {
    ...
    for (Tqstub e : o) {
        array.add(TqstubConverter.getStruct(e, connection));
    }
    return new ARRAY(arrayDescriptor, connection, array.toArray());
}

    public static Tqstub getObject(Struct struct) throws SQLException {
    if (struct == null || struct.getAttributes() == null || struct.getAttributes().length == 0) {
        return null;
    }
    Tqstub result = new Tqstub();
    ...
    public static List<Tqstub> getObjectList(Array array) throws SQLException {
    List<Tqstub> result = new ArrayList<Tqstub>();
    ResultSet rs = null;
    try {
        rs = array.getResultSet();
        while (rs.next()) {
            result.add(TqstubConverter.getObject((Struct) rs.getObject(2)));
        }
    } finally {
        if (rs != null) {
            rs.close();
