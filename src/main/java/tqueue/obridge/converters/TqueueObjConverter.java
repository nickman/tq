...
public final class TqueueObjConverter {

    public static final String TYPE_NAME = "TQUEUE_OBJ";

    private TqueueObjConverter() {
    }

    public static Struct getStruct(TqueueObj o, Connection connection) throws SQLException {
    if (o == null) {
        return null;
    }
    ...
    struct.add(13, TqueueObjConverter.getStruct(o.getTradeToCorrect(), connection)); // TRADE_TO_CORRECT
    return connection.createStruct(TYPE_NAME, struct.toArray());
}

    public static Array getListArray(List<TqueueObj> o, Connection c, String typeName) throws SQLException {
    ...
    for (TqueueObj e : o) {
        array.add(TqueueObjConverter.getStruct(e, connection));
    }
    return new ARRAY(arrayDescriptor, connection, array.toArray());
}

    public static TqueueObj getObject(Struct struct) throws SQLException {
    if (struct == null || struct.getAttributes() == null || struct.getAttributes().length == 0) {
        return null;
    }
    TqueueObj result = new TqueueObj();
    ...
    if (attr[13] != null) {
        result.setTradeToCorrect(TqueueObjConverter.getObject((Struct)attr[13])); // TRADE_TO_CORRECT
    }
    return result;
}

    public static List<TqueueObj> getObjectList(Array array) throws SQLException {
    List<TqueueObj> result = new ArrayList<TqueueObj>();
    ResultSet rs = null;
    try {
        rs = array.getResultSet();
        while (rs.next()) {
            result.add(TqueueObjConverter.getObject((Struct) rs.getObject(2)));
        }
    } finally {
        if (rs != null) {
            rs.close();
