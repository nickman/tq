...
GRANT SELECT ON v_$STATNAME to TQREACTOR;
...
GRANT EXECUTE ON DBMS_SQL TO TQREACTOR;
GRANT EXECUTE ON DBMS_SESSION TO TQREACTOR;
...
BEGIN
  DBMS_NETWORK_ACL_ADMIN.CREATE_ACL(acl=>'tqacl.xml',
    description=>'access control list for utl_http',
    principal=>'TQREACTOR',
    is_grant=>TRUE,
    privilege=>'connect');
  commit;
  DBMS_NETWORK_ACL_ADMIN.ASSIGN_ACL(acl=>'tqacl.xml',host=>'*');
  commit;
END;
/
