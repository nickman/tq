create or replace PACKAGE BODY TQV AS
  ...
  */
  ...
  -- TCP Logging
  -- =================================================================
  c  utl_tcp.connection := null;
  tcpRet pls_integer; 
  tcpOpen BOOLEAN := FALSE;
  -- =================================================================
  ...
  -- ==== Streaming Cursor
  -- =====================================================================
  cursor sqx (STARTING_ID INT, MAX_ROWS INT, MAX_BATCH_SIZE INT) is SELECT VALUE(T) FROM TABLE (
    TQV.TRADEBATCH(
...
--
  ...
  FUNCTION STREAMTBATCHES(STARTING_ID IN INT DEFAULT 0, MAX_ROWS IN INT DEFAULT 5000, MAX_BATCH_SIZE IN INT DEFAULT 10) RETURN TQBATCH_ARR PIPELINED PARALLEL_ENABLE IS
      batchy TQBATCH;
    BEGIN
      IF(sqx%ISOPEN = FALSE) THEN
        open sqx(STARTING_ID, MAX_ROWS, MAX_BATCH_SIZE);
        streamedRows := 0;
      END IF;
      fetch sqx into batchy;
      IF(sqx%NOTFOUND = FALSE) THEN
        pipe row(batchy);
        streamedRows := streamedRows + 1;
        TLOG('PIPED BATCH: ' || streamedRows);   
      END IF;
      
    RETURN;
  END STREAMTBATCHES;
  
  PROCEDURE CLOSESTREAM IS
  BEGIN
    streamedRows := 0;
    IF(sqx%ISOPEN) THEN
      close sqx;
  ...
  FUNCTION STREAMCURSOR(STARTING_ID IN INT DEFAULT 0, MAX_ROWS IN INT DEFAULT 5000, MAX_BATCH_SIZE IN INT DEFAULT 10) RETURN StreamCursorTyp IS
    scur StreamCursorTyp;
  BEGIN
    OPEN scur FOR 
      select account, tcount, first_t, last_t, batch_id, TQV.STUBTOSTR(STUBS) from TABLE(TQV.STREAMTBATCHES(STARTING_ID, MAX_ROWS, MAX_BATCH_SIZE)) ORDER BY FIRST_T;
      -- select account, tcount, first_t, last_t, batch_id, TQV.STUBTOSTR(STUBS) from TABLE(TQV.QUERYTBATCHES(STARTING_ID, MAX_ROWS, MAX_BATCH_SIZE, 0)) ORDER BY FIRST_T;
    RETURN scur;
  END STREAMCURSOR;
      ...
      WHILE(1 = 1) LOOP
        open qx;
          LOOP
            ...
            tlogevent('PIPED BATCH: ' || pipedRows);
          END LOOP;
        close qx;
